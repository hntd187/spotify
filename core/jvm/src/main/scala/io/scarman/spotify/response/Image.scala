...
case class Image(height: Int, url: String, width: Int) {
  ...
  def download(location: String)(implicit backend: SttpBackend[Future, Any], ec: ExecutionContext): Future[DownloadResults] = {
    download(location, None)
  }

  def download(location: String, checksum: Option[String])(implicit
      backend: SttpBackend[Future, Any],
      ec: ExecutionContext
  ): Future[DownloadResults] = {
