...
class ArtistSpec extends UnitSpec {
  ...
  describe("Tests for Artist Endpoints") {
    it("Should get Pitbull's sweet music") {
      ...
      relatedDudes.map(_().last.name `shouldBe` "Jennifer Lopez")
    }

    it("Should get Pitbull's platinum albums") {
      val request = spotify.getArtist(id = "0TnOYISbd1XYRBk9myaseg").albums(AlbumType.album)
      val albums  = request()
      ...
      val artistAlbums = spotify.getArtistAlbums(id = "0TnOYISbd1XYRBk9myaseg", albumType = List(AlbumType.album))
      val aa           = artistAlbums()
